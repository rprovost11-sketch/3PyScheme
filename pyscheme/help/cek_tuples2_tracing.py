    ...
    while True:
        ...
        while True:
            ...
            if tag == EXPR_INT:
                # C = (EXPR_INT, n)
                V = (VAL_INT, C[1])
                if trace: _trace("EVAL/EXPR_INT", C, V, E, K)
                break

            elif tag == EXPR_VAR:
                # C = (EXPR_VAR, name)
                V = env_lookup(E, C[1])
                if trace: _trace("EVAL/EXPR_VAR", C, V, E, K)
                break

            elif tag == EXPR_LAM:
                # C = (EXPR_LAM, param, body)
                V = (VAL_CLOSURE, C[1], C[2], E)
                if trace: _trace("EVAL/EXPR_LAM", C, V, E, K)
                break
                ...
                C = C[1]
                if trace: _trace("EVAL/EXPR_APP", C, V, E, K)
                ...
                C = C[1]
                if trace: _trace("EVAL/EXPR_IF", C, V, E, K)
                # no break - keep descending, still in EVAL
        ...
        while True:
            if not K:
                if trace: _trace("APPLY/RETURN", C, V, E, K)
                return V
            ...
            if ftag == FRAME_IF:
                ...
                if V[0] == VAL_INT and V[1] == 0:
                    ...
                    C = frame[1]
                E = frame[3]
                if trace: _trace("APPLY/FRAME_IF", C, V, E, K)
                break
                ...
                K.append((FRAME_CALL, V))   # V is the FRAME_ARG tuple constructed in the EXPR_APP case above
                C = frame[1]
                E = frame[2]
                if trace: _trace("APPLY/FRAME_ARG", C, V, E, K)
                break
                ...
                C = body
                if trace: _trace("APPLY/FRAME_CALL", C, V, E, K)
                break

...
_SCHEME_BASE_NAMES = [
    ...
    'bytevector-copy', 'bytevector-copy!', 'utf8->string', 'string->utf8',
    # Other (scheme base) procedures.
    'exact-integer-sqrt', 'features',
    # Input and output (R7RS 6.13).  These exist as global primitives but were not
    # exported by any library, so define-library code (e.g. an SRFI written in
    # Scheme) could not reach them; (scheme base) is their R7RS home.  Names not
    # implemented are silently skipped by _register_filtered.
    'call-with-port',
...
if __name__ == '__main__':
    ...
    check('only filter',        resolve_import_set(
        only_form) == {'a': 1, 'c': 3})
    ...
    check('except filter',      resolve_import_set(
        except_form) == {'a': 1, 'c': 3})
    ...
    check('rename keeps b,c',   renamed.get(
        'b') == 2 and renamed.get('c') == 3)

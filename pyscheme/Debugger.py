...
class StepHook:
    ...
    def __init__(self):
        # len(K) threshold; None means step every expr
...
class Debugger:
        ...
        while current is not None and current is not global_env:
            ...
            if names_in_scope:
                ...
                while i < len(names_in_scope):
                    ...
                    print(symbol_name(sid) + ':   ' +
                          pretty_print(current._bindings[sid]))
                    i = i + 1
    ...
    def _walk_form(cell, callable_name, count, target_n):
        ...
        arg = cell.cdr
        while is_cons(arg):
            result = Debugger._walk_form(
                arg.car, callable_name, count, target_n)
    ...
    def _find_nth_call(body_cons, callable_name, target_n):
        ...
        while is_cons(forms):
            result = Debugger._walk_form(
                forms.car, callable_name, count, target_n)
            if result is not None:
                return result
            forms = forms.cdr
        return None
    ...
    def _ann_fmt(cell, annotations, ind):
        ...
        while is_cons(cur):
            lines.append(
                pad + Debugger._ann_fmt(cur.car, annotations, child_ind))
            cur = cur.cdr
        if not is_nil(cur):
            lines.append(
                pad + '. ' + Debugger._ann_fmt(cur, annotations, child_ind))
    ...
    def __init__(self):
        ...
        # id(cons_node) -> [display, cond, node, fn_name, fn_obj]
        self._inner_targets = {}
        self._disabled = set()   # display names of disabled breakpoints
        # ordered names from last `b` listing (for b! N / b- N)
        self._bp_index = []
    ...
    def _resolve_bp_ref(self, ref):
        ...
        if ref.isdigit():
            ...
            if idx < 1 or idx > len(self._bp_index):
                print('No breakpoint #' + str(idx) +
                      '.  Use b to list breakpoints.')
                return None
        ...
        if cmd == 'b':
            if rest == '' or rest == '*':
                ...
                while i < len(bp_names):
                    ...
                    if disabled:
                        print('  ' + str(idx) + ': ' + _DIM +
                              name + tag + '  [disabled]' + _RESET)
                    else:
                        print('  ' + str(idx) + ': ' +
                              _BOLD + name + _RESET + tag)
                    idx = idx + 1
                    i = i + 1
                ...
                while i < len(inner_pairs):
                    ...
                    if disabled:
                        print('  ' + str(idx) + ': ' + _DIM +
                              display + tag + '  [disabled]' + _RESET)
                    else:
                        print('  ' + str(idx) + ': ' +
                              _BOLD + display + _RESET + tag)
                    idx = idx + 1
                    i = i + 1
                if self.break_on:
                    has_any = True
            ...
            for ref in rest.split():
                ...
                if name in self.breakpoints:
                    ...
                    for nid in self._inner_targets:
                        entry = self._inner_targets[nid]
                        if entry[0] == name:
                            self._inner_targets[nid] = [
                                entry[0], None, entry[2], entry[3], entry[4]]
                            print('Condition cleared on ' + name + '.')
                            found = True
                            break
                    if not found:
        ...
        while i < len(pairs):
            ...
            if name in self.breakpoints:
                ...
                for nid in self._inner_targets:
                    ...
                    if entry[0] == name:
                        self._inner_targets[nid] = [
                            entry[0], cond, entry[2], entry[3], entry[4]]
                        print('Condition set on ' + name + ' :when ' + cond)
                        found = True
                        break
                if not found:
        ...
        if cmd == 'w-':
            if rest == '':
                ...
                if idx < 1 or idx > len(self.watch_list):
                    print('No watch #' + str(idx) +
                          '.  Use w to list watches.')
                else:
    ...
    def _set_inner_breakpoint(self, spec, cond, env):
        ...
        if index == '?':
            ...
            while k < len(calls):
                ...
                k = k + 1
            print('Body of ' + fn_name + ' - calls to ' +
                  call_name + ' numbered:')
            forms = body
            while is_cons(forms):
            ...
            for tok in choice.split():
                ...
                if n < 1 or n > len(calls):
                    print('Selection out of range: ' + str(n) +
                          ' (1-' + str(len(calls)) + ').')
                    return
                indices.append(n)
            k = 0
            while k < len(indices):
                ...
                self._inner_targets[id(target)] = [
                    display, cond, target, fn_name, fn]
                self._disabled.discard(display)
                if cond is None:
        ...
        if target is None:
            print('No call to ' + call_name + ' at index ' +
                  str(index) + ' in ' + fn_name + '.')
            return

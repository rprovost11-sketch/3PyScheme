...
def format_with_caret(msg, src):
    ...
    if src is None:
        return msg
    if src.filename:
        prefix = '"%s" line %d, col %d: %s' % (
            src.filename, src.line, src.col, msg)
    else:
...
if __name__ == '__main__':
    ...
    check('SourceInfo fields',  si.line == 3 and si.col ==
          5 and si.source_line == 'abc' and si.filename is None)
    ...
    check('special prim kind',  as_primitive_kind(
        make_primitive('apply', dummy_fn)) == PRIM_APPLY)

    # Case closure
    case_cls = make_case_closure([(('x',), NIL_VALUE, None)], None, 'doc')
    check('is_case_closure',          is_case_closure(case_cls))
    check('as_case_closure_clauses',  len(
        as_case_closure_clauses(case_cls)) == 1)
    check('as_case_closure_env',      as_case_closure_env(case_cls) is None)
    check('as_case_closure_docstring',
          as_case_closure_docstring(case_cls) == 'doc')
    ...
    check('continuation handler_snapshot',
          as_continuation_handlers(cont) is hsnap)
    check('continuation shadow_snapshot',
          as_continuation_shadow(cont) is ssnap)
    ...
    check('syntax_transformer name',
          as_syntax_transformer_name(st) == 'swap!')
    check('syntax_transformer literals',
          as_syntax_transformer_literals(st) == ['set!'])
    check('syntax_transformer ellipsis',
          as_syntax_transformer_ellipsis(st) == '...')
    check('syntax_transformer rules len',    len(
        as_syntax_transformer_rules(st)) == 1)
    ...
    check('as_error_object_irritants len', len(
        as_error_object_irritants(eo)) == 2)
    ...
    check('record_type field_names',
          as_record_type_field_names(rt) == ['x', 'y'])
    ...
    check('list structure', is_cons(L) and is_cons(cdr(L))
          and is_cons(cdr(cdr(L))) and is_nil(cdr(cdr(cdr(L)))))
    check('list elements',  car(L) == 10 and car(
        cdr(L)) == 20 and car(cdr(cdr(L))) == 30)

    # format_with_caret
    out = format_with_caret(
        'bad thing', SourceInfo(1, 3, 'hello', REPL_FILENAME))
    check('3-line render with filename',    out ==
          '"<repl>" line 1, col 3: bad thing\nhello\n  ^')
    out = format_with_caret('bad thing', SourceInfo(1, 3, 'hello', None))
    check('3-line render no filename',      out ==
          'line 1, col 3: bad thing\nhello\n  ^')
    out = format_with_caret('bad', SourceInfo(2, 5, '', REPL_FILENAME))
    check('1-line fallback with filename',
          out == '"<repl>" line 2, col 5: bad')
